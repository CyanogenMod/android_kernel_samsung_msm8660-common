...
void yda165_speaker_voip_onoff(int onoff) /* speaker voip path amp onoff */
{
	D4HP3_SETTING_INFO stInfo;
#if AMPREG_DEBUG	
	unsigned char buf;
	...
	if (onoff) //on
	{
		...
		cur_mode = 5;
		pr_info(MODULE_NAME ":speaker voip on[cur_mode:%d]\n", cur_mode);
	...
	{
		pr_info(MODULE_NAME ":speaker voip off\n");
...
void yda165_headset_call_onoff(int onoff) /* headset call path amp onoff */
{
	D4HP3_SETTING_INFO stInfo;
