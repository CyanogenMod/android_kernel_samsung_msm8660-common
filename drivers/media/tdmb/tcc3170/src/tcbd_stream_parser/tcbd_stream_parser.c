...
#define SIZE_MSC_CACHE_BUFF	(188*10)
#define SIZE_FIC_CACHE_BUFF	(388*4)
#define SIZE_STATUS_CACHE_BUFF (32*4)

#if defined(__CALLBACK_BUFFER_HEADER__)
#define IDX_TYPE 0
#define IDX_SUBCH 1
#define IDX_SIZE 2
#define SIZE_MSC_STACK_BUFF	(1024*16 + SIZE_BUFF_HEADER)
#define SIZE_FIC_STACK_BUFF	(388*10 + SIZE_BUFF_HEADER)
#define SIZE_STATUS_STACK_BUFF (32*10 + SIZE_BUFF_HEADER)
#else /*__CALLBACK_BUFFER_HEADER__*/
#define SIZE_MSC_STACK_BUFF	(1024*16)
#define SIZE_FIC_STACK_BUFF	(388*10)
#define SIZE_STATUS_STACK_BUFF (32*10)
#endif /*!__CALLBACK_BUFFER_HEADER__*/
	...
	struct tcbd_split_stream_data *_parser, u8 *_buffer)
{
...
#if defined(__MERGE_EACH_TYPEOF_STREAM__)
	static u8
		msc_buff[TCBD_MAX_NUM_SERVICE][SIZE_MSC_STACK_BUFF];
	...
	case DATA_TYPE_MSC:
		...
		if (quick_msc_idx >= TCBD_MAX_NUM_SERVICE)
			tcbd_debug(DEBUG_ERROR,
				"quick_msc_idx:%d, header->subch:%d\n",
		...
		if (merged->curr_pos >= SIZE_MSC_STACK_BUFF) {
			tcbd_debug(DEBUG_ERROR, "too many cif!!\n");
			return;
		}
		break;
	case DATA_TYPE_FIC:
...
#if defined(__CALLBACK_BUFFER_HEADER__)
	merged->buffer += SIZE_BUFF_HEADER;
#endif /* __CALLBACK_BUFFER_HEADER__ */
	...
	if (_parser->next_read) {
		if (_parser->state != STATE_GARBAGE) {
			...
			tcbd_debug(DEBUG_PARSING_PROC,
				"send %s data %d bytes, pre:%d, curr:%d, buffer:%p\n",
					type[header->type],
					header->data_size,
					pre_copied,
					_parser->next_read,
...
static s32 tcbd_push_stream(struct tcbd_split_stream_data *_parser)
{
	...
	for (i = 0; i < _parser->num_subch; i++) {
		...
		if (_parser->stream_callback) {
#if defined(__CALLBACK_BUFFER_HEADER__)
			buffer = merged->buffer-SIZE_BUFF_HEADER;
			buffer[IDX_TYPE] = DATA_TYPE_MSC;
			buffer[IDX_SUBCH] = merged->subch_id;
			*((u16 *)&buffer[IDX_SIZE]) = (u16)merged->curr_pos;
			tot_size = merged->curr_pos + SIZE_BUFF_HEADER;
#else  /* __CALLBACK_BUFFER_HEADER__ */
			buffer = merged->buffer;
			tot_size = merged->curr_pos;
#endif /* !__CALLBACK_BUFFER_HEADER__ */
			_parser->stream_callback(
						buffer,
						tot_size,
...
#endif /* !__CALLBACK_BUFFER_HEADER__ */
			_parser->stream_callback(
				buffer, tot_size, merged->subch_id, i);
			merged->buffer = NULL; merged->curr_pos = 0;
		}
	}
	return 0;
}
