...
#if defined (CONFIG_KOR_MODEL_SHV_E120K)|| defined (CONFIG_KOR_MODEL_SHV_E120S)|| defined (CONFIG_KOR_MODEL_SHV_E120L)
...
static irqreturn_t touchkey_interrupt(int irq, void *dummy)  // ks 79 - threaded irq(becuase of pmic gpio int pin)-> when reg is read in work_func, data0 is always release. so temporarily move the work_func to threaded irq.
{
	...
	if (data[0] & UPDOWN_EVENT_BIT) {
		if(press_check == (data[0] & KEYCODE_BIT)){
			...
			input_report_key(touchkey_driver->input_dev, touchkey_keycode[press_check], 0);
			touchkey_pressed &= ~(1 << press_check);
			input_sync(touchkey_driver->input_dev);
	        }
			press_check = 0;
	} else {
		if (is_touchkey_available() != 1) {   
			...
			touchkey_pressed |= (1 << (data[0] & KEYCODE_BIT));
			input_sync(touchkey_driver->input_dev);
			press_check = data[0] & KEYCODE_BIT;
			if(g_debug_switch)				
