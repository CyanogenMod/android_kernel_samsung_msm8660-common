...
{
	pr_debug("[TKEY] sec_touchkey_early_suspend\n");
...
{
	pr_debug("[TKEY] sec_touchkey_early_resume\n");
...
static int i2c_touchkey_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_debug("[TKEY] sec i2c_touchkey_probe\n");
		...
		else if (((data[1] < 0x09) && (data[2] == 0x18))|| ((((data[1] == 0x0) && (data[2] == 0x0) )||((data[1] == 0xff) && (data[2] == 0xff) ))&& (get_hw_rev() >=0x0a )))
{
	...
	if (ret) {
		pr_err("[TKEY] sec touch keypad registration failed, module not inserted.ret= %d\n", ret);
	}
