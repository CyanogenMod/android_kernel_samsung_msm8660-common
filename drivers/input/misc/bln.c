...
static void bln_led_enable(int onoff)
{
	// return if led is already off
	if (onoff == 0 && !bln_led_state) {
		pr_err("%s: already off\n", __FUNCTION__);
		return;
	}

	// return if led is already on
	if (onoff == 1 && bln_led_state) {
		pr_err("%s: already on\n", __FUNCTION__);
		return;
	}
...
static void bln_led_notification_enable(int onoff)
{
	// return if led notification is already disabled
	if (onoff == 0  && !bln_ongoing) {
		pr_err("%s: disable fail (ongoing=%d\n", __FUNCTION__, bln_ongoing);
		return;
	}

	// return if led notification is already enabled
	if (onoff == 1 && !bln_enabled && bln_ongoing && !bln_suspended) {
		pr_err("%s: enable fail (enabled=%d ongoing=%d suspended=%d\n", __FUNCTION__, bln_enabled, bln_ongoing, bln_suspended);
		return;
	}

	// return if led notification is already disabled and unsuspended
	if (onoff == 2 && !bln_suspended && !bln_ongoing) {
		pr_err("%s: cancel activity fail (ongoing=%d suspended=%d\n", __FUNCTION__, bln_ongoing, bln_suspended);
		return;
	}
	...
	// cancel led notification on resume
	case 2:
		bln_blink_enable(0);
		bln_led_enable(0);
		bln_ongoing = false;
		pr_info("%s: cancel activity success\n", __FUNCTION__);
		break;
...
static void bln_late_resume(struct early_suspend *h)
{
	...
	// cancel bln activity upon resume
	bln_led_notification_enable(2);
