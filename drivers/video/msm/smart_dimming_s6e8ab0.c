...
/*
	gamma_regs[0] = 0xFA;
	gamma = brightness; //30
	calc_gamma_table(&lcd->smart, gamma, gamma_regs+1); // */
u32 calc_gamma_table(struct str_smart_dim *smart, u32 gv, u8 result[])
{
	u32 i, c;
	u32 temp;
	u32 lidx;
	u32 dv[CI_MAX][IV_MAX]; // 3, 7
	s16 gamma[CI_MAX][IV_MAX];//3, 7
	u16 offset;
 	u32(*calc_reg[IV_MAX])(int ci, u32 dv[CI_MAX][IV_MAX]) = {// 7
		calc_v1_reg,
		calc_v15_reg,
		calc_v35_reg,
		calc_v50_reg,
	...
	memset(gamma, 0, sizeof(gamma)); // 16BIT , 21 ARRAY

	for (c = CI_RED; c < CI_MAX; c++) 
		dv[c][0] = smart->adjust_volt[c][AD_IV1]; //[ 0 ~ 3 (RED, GREEN, BLUE, MAX)][1]


	for (i = IV_15; i < IV_MAX; i++) { // 1 ~ 7
		temp = smart->g22_tbl[dv_value[i]] * gv;  // huggac_p
