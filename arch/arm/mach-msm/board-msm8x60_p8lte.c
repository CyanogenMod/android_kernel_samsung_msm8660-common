...
#ifdef CONFIG_SENSORS_K3DH_P5LTE
...
static struct platform_device mipi_dsi_s6e8ab0_wxga_panel_device = {
...
static struct p8_charging_status_callbacks {
	void	(*tsp_set_charging_cable) (int type);
} charging_cbs;

static bool is_cable_attached;

static void mxt768e_power_on(void);
static void mxt768e_power_off(void);
...
void tsp_set_charging_cable(bool set)
{
    if (charging_cbs.tsp_set_charging_cable)
    {
        printk("tsp_set_charging_cable %d\n", set);
        if (set)
...
struct regulator *mxt768e_l8;
struct regulator *mxt768e_l19;

static void mxt768e_power_init(void)
{
	int ret;

	mxt768e_l19 = regulator_get(NULL/*&rpm_vreg_device[RPM_VREG_ID_PM8058_L19].dev*/, "8058_l19");
	ret = regulator_set_voltage(mxt768e_l19, 3000000, 3000000);
	if (ret) {
		printk("%s: error setting voltage\n", __func__);
	}

	mxt768e_l8 = regulator_get(NULL/*&rpm_vreg_device[RPM_VREG_ID_PM8058_L8].dev*/, "8058_l8");
	ret = regulator_set_voltage(mxt768e_l8, 1800000, 1800000);
	if (ret) {
		printk("%s: error setting voltage\n", __func__);
	}
}
...
static void mxt768e_power_on(void)
{
	...
	printk("[TSP] %s, %d\n",__func__,__LINE__);
	ret = regulator_enable(mxt768e_l19);
	if (ret) {
		printk("%s: error enabling regulator\n", __func__);
	}
	mdelay(50);

	ret = regulator_enable(mxt768e_l8);
	if (ret) {
		printk("%s: error enabling regulator\n", __func__);
	}
	mdelay(50);
...
static void mxt768e_power_off(void)
{
	...
	printk("[TSP] %s, %d\n",__func__,__LINE__);
	ret = regulator_disable(mxt768e_l19);
	if (ret) {
		printk("%s: error enabling regulator\n", __func__);
	}
	ret = regulator_disable(mxt768e_l8);
	if (ret) {
		printk("%s: error enabling regulator\n", __func__);
	}
}
...
#else
		.platform_data = NULL, //&p3_touch_platform_data,
#endif
	},
};
...
int get_jig_gpio_to_sys(void)
{
	if(system_rev >= 0x4)
		return PM8058_GPIO_PM_TO_SYS(PM8058_GPIO(6));
	else
		return PM8058_GPIO_PM_TO_SYS(PM8058_GPIO(2));
}
...
static struct msm_bus_vectors mdp_sd_ebi_vectors[] = {
	/* Default case static display/UI/2d/3d if FB SMI */
	{
		.src = MSM_BUS_MASTER_MDP_PORT0,
		.dst = MSM_BUS_SLAVE_SMI,
#ifndef CONFIG_FB_MSM_MIPI_S6E8AB0_WXGA_PANEL // test		
		.ab = 0,
		.ib = 0,
#else
		.ab = 740000000,
...
#define MLCD_ON 128
#define AMOLED_RESET 28

static void mipi_S6E8AB0_panel_reset_up(void)
 {
	 gpio_tlmm_config(GPIO_CFG(AMOLED_RESET,  0, GPIO_CFG_OUTPUT,GPIO_CFG_NO_PULL, GPIO_CFG_2MA),1); 
	 gpio_set_value(AMOLED_RESET, 1);
 }

static void mipi_S6E8AB0_panel_reset_down(void)
 {
	 gpio_set_value(AMOLED_RESET, 0);
   	 gpio_tlmm_config(GPIO_CFG(AMOLED_RESET,  0, GPIO_CFG_OUTPUT,GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),1); 
 }

 static int mipi_S6E8AB0_panel_power(int enable)
 {
	 static struct regulator *l12a = NULL;
	 static struct regulator *l17a = NULL;
	 static int panel_initialized = 0;
	 int ret;
 
	 printk("[Mipi_LCD] %s:enable:%d\n", __FUNCTION__, enable);
	
	 /*  1. MLCD_ON GPIO Enable
	  *  2. VREG_L3B Enable
	  *  3. VREG_L1B Enable
	  *  */
	 if(!panel_initialized) {
		 l12a = regulator_get(NULL/*&rpm_vreg_device[RPM_VREG_ID_PM8058_L12].dev*/, "8058_l12");
		 l17a = regulator_get(NULL/*&rpm_vreg_device[RPM_VREG_ID_PM8058_L17].dev*/, "8058_l17");

		 ret = regulator_set_voltage(l12a, 1800000, 1800000);
		if (ret) {
			printk("%s: set voltage error \n", __func__);
		}		 
		 ret = regulator_set_voltage(l17a, 3000000, 3000000);
		if (ret) {
			printk("%s: set voltage error \n", __func__);
		}
	 ...
	 if(enable) {
		...
		gpio_set_value(MLCD_ON, 1);
#endif
		printk("[MIPI] MIPI_DSI S6E8AB0 Panel Power ON\n");
		ret = regulator_enable(l12a);
		if (ret) {
			printk("%s: error enabling regulator\n", __func__);
		}

		ret = regulator_enable(l17a);
		if (ret) {
			printk("%s: error enabling regulator\n", __func__);
		}
		...
		msleep(5);
		if(p8lte_has_cmc624()) {
	        mipi_S6E8AB0_panel_reset_down();
			msleep(1);
	        mipi_S6E8AB0_panel_reset_up();
			msleep(1);
	        mipi_S6E8AB0_panel_reset_down();
			msleep(1);
	        mipi_S6E8AB0_panel_reset_up();
...
#endif
		 printk("[MIPI] MIPI_DSI S6E8AB0 Panel Power OFF\n");
         
		 mipi_S6E8AB0_panel_reset_down();
		 msleep(5);

		 ret = regulator_disable(l17a);
		 if (ret) {
				 printk("%s: error enabling regulator\n", __func__);
		 }

		 ret = regulator_disable(l12a);
		 if (ret) {
				 printk("%s: error enabling regulator\n", __func__);
		 }
		...
		if(p8lte_has_cmc624()) {
			...
			gpio_tlmm_config(GPIO_CFG(IMA_PWR_EN,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),GPIO_CFG_DISABLE);
			gpio_tlmm_config(GPIO_CFG(IMA_SLEEP,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),GPIO_CFG_DISABLE);
			gpio_tlmm_config(GPIO_CFG(IMA_CMC_EN,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),GPIO_CFG_DISABLE);

			gpio_tlmm_config(GPIO_CFG(IMA_I2C_SDA,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),GPIO_CFG_DISABLE);
			gpio_tlmm_config(GPIO_CFG(IMA_I2C_SCL,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),GPIO_CFG_DISABLE);
		} else {
			 gpio_set_value(MLCD_ON, 0);
			 gpio_tlmm_config(GPIO_CFG(MLCD_ON,  0, GPIO_CFG_OUTPUT,GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),1);           
		}
#else
		 gpio_set_value(MLCD_ON, 0);
		 gpio_tlmm_config(GPIO_CFG(MLCD_ON,  0, GPIO_CFG_OUTPUT,GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),1);           
...
#ifdef CONFIG_FB_MSM_MIPI_DSI
#ifdef CONFIG_FB_MSM_MIPI_S6E8AB0_WXGA_PANEL // test
